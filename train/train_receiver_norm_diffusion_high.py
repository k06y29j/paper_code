#!/usr/bin/env python
"""Conditional diffusion for receiver-normalized high Swin latents.
...
diffusion model only generates the missing 12 high channels; the transmitted
4-channel observation is never rewritten.
"""
...
def setup_log_file(path: str | None):
    ...
    builtins.print(f"\n=== Receiver-normalized high diffusion @ {time.strftime('%Y-%m-%d %H:%M:%S')} ===")
...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, t: torch.Tensor, num_steps: int) -> torch.Tensor:
        ...
        # Use the raw discrete timestep. Normalizing t to [0, 1] makes most
        # sinusoidal channels nearly constant for a 1000-step diffusion chain.
        _ = num_steps
...
class DiffHighUNet(nn.Module):
    """Latent U-Net for eps prediction on 12 high channels conditioned by z0_rx."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train high-latent diffusion conditioned on receiver-normalized z0",
    ...
    p.add_argument("--lambda_x0", type=float, default=0.05)
    p.add_argument("--clip_x0", type=float, default=0.0, help="Clamp sampled/diagnostic x0 predictions; <=0 disables")
    p.add_argument("--unet_base", type=int, default=128)
    p.add_argument("--unet_depth", type=int, default=3)
    p.add_argument("--time_dim", type=int, default=256)
    p.add_argument("--ddim_steps_a", type=int, default=10)
    p.add_argument("--ddim_steps_b", type=int, default=20)
    p.add_argument("--sample_init", type=str, default="noise", choices=["noise", "zero"])
    p.add_argument("--diag_timesteps", type=str, default="50,100,200", help="Comma-separated one-step x0 diagnostic timesteps")
    ...
    p.add_argument("--save_dir", type=str, default="checkpoints-ar/diffusion_high")
    p.add_argument("--log_file", type=str, default="checkpoints-ar/diffusion_high/train.log")
    return p.parse_args()
...
def diffusion_loss(
    ...
    with make_autocast(target.device, amp_enabled, amp_dtype):
        eps_pred = model(z_t, cond, t).float()
...
def ddim_sample(
    ...
    step_ids = torch.linspace(num_train_steps - 1, 0, max(1, int(steps)), device=cond.device).round().long()
    ...
    for i, ti in enumerate(step_ids):
        ...
        with make_autocast(cond.device, amp_enabled, amp_dtype):
            eps_pred = model(z, cond, t).float()
        x0_pred = (z - s_t * eps_pred) / torch.sqrt(a_t).clamp_min(1e-8)
        if float(clip_x0) > 0:
        ...
        if i == len(step_ids) - 1:
            ...
            next_t = step_ids[i + 1]
            a_next = alpha_bar[int(next_t.item())].view(1, 1, 1, 1).to(device=cond.device, dtype=z.dtype)
        z = torch.sqrt(a_next) * x0_pred + torch.sqrt((1.0 - a_next).clamp_min(0.0)) * eps_pred
    return z.float()


def parse_diag_timesteps(text: str, max_steps: int) -> list[int]:
    out: list[int] = []
...
def save_checkpoint(
    ...
    payload = {
        ...
        "stage": "03_diffusion_high_unet_frozen_swin",
        "epoch": int(epoch),
        "metrics": metrics,
        "args": vars(args),
...
def main() -> None:
    ...
    print(f"device={device}, amp={amp_dtype if amp_enabled else 'fp32'}, stage=03_diffusion_high_unet_frozen_swin, snr={float(args.snr_db):g}dB")
    ...
    diag_timesteps = parse_diag_timesteps(str(args.diag_timesteps), int(args.diffusion_steps))
    print(
        ...
        f"T={args.diffusion_steps} schedule={args.noise_schedule} lambda_x0={float(args.lambda_x0):g} "
        f"clip_x0={float(args.clip_x0):g} ddim={args.ddim_steps_a}/{args.ddim_steps_b} "
        f"init={args.sample_init} diag_t={diag_timesteps} aat_error={aat_err:.3e}"
    )
    ...
    best = -1.0
    best_name = "diffusion_high_unet_frozen_swin_best.pth"
    latest_name = "diffusion_high_unet_frozen_swin_latest.pth"
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if do_eval:
            ...
            print(
                ...
                f"base={metrics['val_psnr_base']:.4f} recv{args.ddim_steps_a}={metrics['val_psnr_recv_a']:.4f} "
                f"recv{args.ddim_steps_b}={metrics['val_psnr_recv_b']:.4f} oracle={metrics['val_psnr_oracle']:.4f} "
    ...
    print(f"best_val_psnr_recv{args.ddim_steps_b}={best:.4f}")
