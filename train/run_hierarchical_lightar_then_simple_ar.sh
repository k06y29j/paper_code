...
STAGE1_DIR="${ROOT}/01_light_ar_joint_snr${SNR}"
STAGE2_DIR="${ROOT}/02_simple_h256d6_frozen_snr${SNR}"
...
log_root "start SNR=${SNR}dB GPU=${GPU}"
log_root "stage1 joint light AR -> ${STAGE1_DIR}"
env STAGE="joint" \
...
log_root "stage2 frozen Swin simple h256d6 -> ${STAGE2_DIR}"
env INIT_HIER_CKPT="${INIT_CKPT}" \
  ...
  train/run_hierarchical_swin_ar_awgn12.sh "${GPU}" "${STAGE2_DIR}" \
    --snr_db "${SNR}" --baseline_psnr nan --lambda_recv 1.0 --lambda_ar 0.2 \
  2>&1 | tee -a "${STAGE2_DIR}/train.stdout"

log_root "done SNR=${SNR}dB"
