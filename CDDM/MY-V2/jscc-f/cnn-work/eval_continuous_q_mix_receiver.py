...
under ``cnn-work/results-receiver``.
"""
...
def validate_checkpoint(payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if str(payload.get("stage", "")) != "explore2_continuous_q_mix_receiver":
        raise ValueError("--checkpoint is not a supported fixed q-mix receiver checkpoint")
...
def _report_output_path(
    ...
    try:
        ...
        raise ValueError("--output must remain under MY-V2/jscc-f/cnn-work") from error
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        default="",
        help="saved fixed-q-mix receiver checkpoint to inspect read-only",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="",
        help="JSON path relative to cnn-work; default=results-receiver/<checkpoint>_strict_fullval.json",
    )
