...
INIT="$ROOT/MY-V2/jscc-f/explore-2/checkpoints-fsq-generation/cnn-fsq-k125-paper-joint-ar-v1/fsq_receiver_ar_joint_index_z1_x1_hard_cnn_d3_l5x5x5_d2ft-oracle_independent-d2_epoch020_valid.pth"
SAVE_ROOT="$ROOT/MY-V2/jscc-f/explore-2/checkpoints-fsq-generation"
LOG_ROOT="$ROOT/MY-V2/jscc-f/explore-2/logs-fsq-generation"

GPU="${GPU:-2}"
VERSION="${VERSION:-cnn-fsq-k125-paper-joint-ar-frozen-sched-v2}"
EPOCHS="${EPOCHS:-80}"
...
exec env CUDA_VISIBLE_DEVICES="$GPU" conda run --no-capture-output -n cddm_ddnm \
  ...
  --ar-history-corruption-start 0.1 \
  --ar-history-corruption-end 0.5 \
  --lambda-index 0.01 \
  --lambda-q 0.05 \
  --lambda-final 1 \
  --lambda-oracle 0.25 \
