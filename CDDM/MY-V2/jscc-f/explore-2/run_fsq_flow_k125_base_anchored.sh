...
BASE="$ROOT/MY-V2/jscc-f/explore-2/checkpoints-125/cnn-fsq-k125-frozen-d2-combiner-cont-v1/fsq_receiver_direct_q_z1_x1_continuous_cnn_d3_l5x5x5_frozen_best.pth"
SAVE_ROOT="$ROOT/MY-V2/jscc-f/explore-2/checkpoints-fsq-generation"
LOG_ROOT="$ROOT/MY-V2/jscc-f/explore-2/logs-fsq-generation"

GPU="${GPU:-0}"
VERSION="${VERSION:-cnn-fsq-k125-flow-cont-base-anchored-v1}"
EPOCHS="${EPOCHS:-80}"
STEPS="${STEPS:-4}"
LR="${LR:-1e-5}"
INIT="${INIT:-}"
...
[[ -f "$BASE" ]] || { echo "missing K125 continuous direct base: $BASE" >&2; exit 2; }

INIT_ARGS=()
if [[ -n "$INIT" ]]; then
  [[ -f "$INIT" ]] || { echo "missing anchored flow init: $INIT" >&2; exit 2; }
...
exec env CUDA_VISIBLE_DEVICES="$GPU" conda run --no-capture-output -n cddm_ddnm \
  ...
  --attention-every 2 \
  --heads 4 \
  --flow-base-checkpoint "$BASE" \
  --flow-train-sample-steps "$STEPS" \
  --flow-sample-steps "$STEPS" \
  --flow-sample-noise zero \
  --flow-sample-seed 20260715 \
