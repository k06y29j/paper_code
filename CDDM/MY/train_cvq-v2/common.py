...
def print_run_header(args, title: str, train_n: int, val_n: int) -> None:
    ...
    if int(getattr(args, "stage", 0)) == 1:
        print(
            ...
            f"+{float(getattr(args, 'lambda_all_z', 0.0)):g}*L_rec_all_z"
            f"+{float(getattr(args, 'lambda_usage_entropy', 0.0)):g}*L_usage_entropy"
            f"+{float(getattr(args, 'lambda_usage_topk', 0.0)):g}*L_usage_topk"
        )
        if hasattr(args, "stage1_encoder_freeze_epochs"):
            print(
                "lr_stage1="
                f"epoch<= {int(args.stage1_encoder_freeze_epochs)}: "
                f"encoder={float(args.stage1_encoder_lr_warm):g}, "
                f"decoder={float(args.stage1_decoder_lr):g}, "
                f"codebook={float(args.stage1_codebook_lr_warm):g}; "
                "after: "
                f"encoder={float(args.stage1_encoder_lr_after):g}, "
                f"decoder={float(args.stage1_decoder_lr):g}, "
                f"codebook={float(args.stage1_codebook_lr_after):g}"
            )
